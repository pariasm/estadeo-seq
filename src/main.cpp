...
int main (int argc, char *argv[])
{
  ...
  int result=read_parameters(
        argc, argv, &video_in, first, last, step, video_out, &out_transform, &in_transform, 
  ...
  if(result)
  {
    ...
    if (nframes < 0) nframes = (last - first)/step + 1;
    ...
    size_t r=read_video(video_in, first, last, step, If, size);
    ...
    write_video(video_out, first, step, If, width, height, nframes, nchannels);
